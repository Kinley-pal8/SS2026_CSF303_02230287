...
Given an array of **N integers** (1 ≤ N ≤ 10⁵), find the **maximum possible value of AND** of any subarray of length exactly **K**.
...
- For each test case:
  - First line: `N K`
  - Second line: `N` integers `A[i]` (0 ≤ A[i] ≤ 10⁹)
...
> Think about checking bits from MSB to LSB.
...
- First line: `N K`
- Second line: `N` space-separated integers `A[i]` (−10⁹ ≤ A[i] ≤ 10⁹)
...
- First line: `N M` (1 ≤ N ≤ 10⁵, 1 ≤ M ≤ 2×10⁵)
- Next `M` lines: `u v w` (1 ≤ u, v ≤ N, 1 ≤ w ≤ 10⁹)
...
- **Pay** `toll[i]` coins and move to the next booth — costs **1 minute**
- **Skip** the booth — costs **2 minutes**, but you can skip **at most K booths** in total

Find the **minimum time** to reach booth N.
